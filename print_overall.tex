...
\chapter{Kokkuvõte}
\begin{itemize}
	\item Väga oluline see, mida Ahti ütleb\sidenote{Lk. \pageref{sisu!inimeseks}.}: hulk noori inimesi tegeles ühel või teisel moel aga \emph{koos teiste väga sarnaste ja sama moodi mõtlevate inimestega} suureks saamisega samal ajal, kui sellesamaga tegeles Eesti Vabariik
	...
	\begin{itemize}
		...
		\item Tõnis: õppida tuleb raskeid asju, lihtsad tulevad iseenesest
	\end{itemize}
	\item Mitmel puhul öeldakse, et "ei programmeeri, sest nägin väga vara tõeliselt häid programmeerijaid" (Henn, Sten) Järeldus: arvutihuvi tuleb teisiti realiseerida?
	...
	\item Õpetajatelt (Meelis, AK, Anto jt.) sai pigem üldist juhendamist ja suunamist ja paradigmat (Tõnis Reimo) kui konkreetset õpet. Aga ka lihtsalt kriitilise massi huviliste kokku toomist (Jaak Loonde Ahti jutus)
	...
	\item Kui oleks olnud infoturve tänases mõistes, poleks väga paljut juhtunud (lapse arvutisse laskmine, piraatmängud!) ja kui poleks olnud piraat-tarkvara, poleks ka väga paljut juhtunud (neid proovides omandati kogemusi ja teadmisi võimalustest)
	\item Mõnevõrra EÜS-i fenomen. Omas seltskonnas oli info puuduse tingimustes lihtsam asju ajada
	\item Folkloor kui väga oluline asi: selle abil levis info, muid allikaid ei olnud (Tõnu klaviatuurijutt). Aga ka folkloor (ja tarkvara) kui valuuta ja kaup
	\item Lihtsus
	\begin{itemize}
		...
		\item Tõnise Polütehnikumi õpetus, kuidas keerukust musta kasti sisse peita
		\item Tõnu: Ma ei saa keerulistest asjadest aru
		\item Sten ja muutunud suhe arvutiga
	\end{itemize}
	\item Ülim usaldus
	\begin{itemize}
	...
	\begin{itemize}
		...
		\item Jaanus
	\end{itemize}
	\item Välismaa connection
	\begin{itemize}
		\item Abi, mis me saime Ronilt ja osutasime Leetu
		\item Vladivostok ja Moskva
	\end{itemize}
	\item Võim masina üle, masina seletamatu tõmme ka täiesti teiste huvidega inimeste suhtes (Asko, Jaanus)
	\item Võtmeinimesed
	\begin{itemize}
		\item Eraldi tasub mainimist Ron Dwight, kes mitmest loost läbi jookseb ja on meid hirmsat moodi aidanud
		\item Isikute kaupa
			\begin{itemize}
